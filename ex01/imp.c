...
int main(void) { int n; 
//n é o número a ser verificado int i; //i é uma variável auxiliar int primo = 1; //primo é uma variável 
//lógica que indica se n é primo ou não
    int i;
    int primo = 1;
...
//verifica se n é maior que 1 
    ...
    for (i = 2; i <= sqrt(n); i++) { //verifica se n é divisível por i 
        if (n % i == 0) { //se for, então n não é primo primo = 0; break; //sai do laço } } //verifica o valor da variável primo 
        if (primo == 1) { //se for verdadeiro, imprime que n é primo 
            printf("%d é primo.\n" n); } 
        else 
        { 
